...
use common\models\DbContactAssign;
...
class ActiveRecord extends \yii\db\ActiveRecord {
                ...
                if (!empty($this->$relation)) {
                    if (is_array($this->$relation)) {
                        foreach ($this->$relation as $temp) {
                            $temp->doNotification = false;
                            $temp->delete();
                        }
                    } else {
                        $this->$relation->doNotification = false;
    ...
    public function getContactAssign () {
        return $this->hasMany(DbContactAssign::className(), ['refId' => 'id'])
    ...
    public function notifications(){
        ...
        if(array_key_exists($this->getClassName(), $names)){
            $name = $names[$this->getClassName()];
            $valid = true;
        }

        if($valid && !empty($this->created) && empty($this->oldAttributes['created'])){
            DbMessage::add([
                'ref' => $this,
                'type' => 'change',
                'message' => $name.' added',
        ...
        if($valid && $this->isChanged('status')) {
            DbMessage::add([
                'ref' => $this,
                'type' => 'status',
                'highlight' => 1,
                'message' => 'Status changed from <strong>'.$this->getListLabel('listStatus', $this->oldAttributes['status']).'</strong> to <strong>'.$this->getListLabel('listStatus', $this->status).'</strong>.',
            ]);
        }
    }
